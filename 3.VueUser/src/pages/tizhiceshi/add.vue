...
<div :style='{"width":"100%","padding":"30px 7% 40px","margin":"0px auto","position":"relative","background":"none"}'>
    <el-form
      ...
      label-width="180px"
    >
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="测试标题" prop="ceshibiaoti">
            <el-input v-model="ruleForm.ceshibiaoti" 
                placeholder="测试标题" clearable :disabled=" false  ||ro.ceshibiaoti"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="内容" prop="neirong">
            <el-input v-model="ruleForm.neirong" 
                placeholder="内容" clearable :disabled=" false  ||ro.neirong"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="测试详情" prop="ceshixiangqing">
            <el-input v-model="ruleForm.ceshixiangqing" 
                placeholder="测试详情" clearable :disabled=" false  ||ro.ceshixiangqing"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="测试时间" prop="ceshishijian">
              <el-date-picker
				  :disabled=" false  ||ro.ceshishijian"
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`tizhiceshi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('tizhiceshi/list', {
							...
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`tizhiceshi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
